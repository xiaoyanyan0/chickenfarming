...
# 2. 新建eef_flag
df['eef_flag'] = np.where(df['eef'] <= 250, 1, 0).astype(str)
flag_col = 'eef_flag'
...
remove_cols = [
    ...
    'Houseid','feed_cost',
    'avg_weight_kg','final_avg_weight_kg'
]
...
X = df.drop(columns=['eef', flag_col])
y = df[flag_col]
