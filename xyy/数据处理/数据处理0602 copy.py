...
def calculate_change_rate(series):
    ...
    """
    # 计算原始变化率
    change_rate = series.diff().abs()  # 用绝对值避免符号影响
...
grouped2['温度1-平均变化'] = grouped2.groupby(['ID_NUM', '日龄'])['温度1-平均'].transform(calculate_change_rate)
grouped2['温度2-平均变化'] = grouped2.groupby(['ID_NUM', '日龄'])['温度2-平均'].transform(calculate_change_rate)
grouped2['温度3-平均变化'] = grouped2.groupby(['ID_NUM', '日龄'])['温度3-平均'].transform(calculate_change_rate)
grouped2['温度4-平均变化'] = grouped2.groupby(['ID_NUM', '日龄'])['温度4-平均'].transform(calculate_change_rate)
grouped2['温度5-平均变化'] = grouped2.groupby(['ID_NUM', '日龄'])['温度5-平均'].transform(calculate_change_rate)
# grouped2[['温度1-平均','温度1-平均变化']]


agg_result2=grouped2.groupby(['ID_NUM', '日龄'])[['温度1-平均变化','温度2-平均变化','温度3-平均变化','温度4-平均变化','温度5-平均变化']].max()

agg_result['温度1-平均变化']=agg_result2['温度1-平均变化']
agg_result['温度2-平均变化']=agg_result2['温度2-平均变化']
agg_result['温度3-平均变化']=agg_result2['温度3-平均变化']
agg_result['温度4-平均变化']=agg_result2['温度4-平均变化']
agg_result['温度5-平均变化']=agg_result2['温度5-平均变化']
...
keep_cols=['ID_NUM', '日龄', '温度1-平均_mean', '温度2-平均_mean', '温度3-平均_mean','温度4-平均_mean',  
           '温度5-平均_mean', '温度1-平均变化', '温度2-平均变化', '温度3-平均变化', '温度4-平均变化', '温度5-平均变化',
           '外部-平均_mean','湿度内部平均_mean', '鸡舍温度-最低_mean', '鸡舍温度-平均_mean', '鸡舍温度-最高_mean', 
           '每日温差', '探头温度标准差','Water','Feed']
...
wide_df.to_csv('./data/data_cleaned/wide_df_0602_2.csv', index=False,encoding='gbk')
HumTem_data_normal.to_csv('./data/data_cleaned/HumTem_data_agg0602_2.csv', index=False,encoding='gbk')
##基本信息等拼接
# wide_df['ID_NUM']
...
all_info_temdata2.to_csv('./data/data_cleaned/all_info_temdata0602_2.csv',index=False,encoding='gbk')
...
HumTem_data_normal=pd.read_csv('./data/data_cleaned/HumTem_data_agg0602_2.csv',encoding='gbk')
HumTem_data_normal.columns.to_list()
# HumTem_data_normal['探头温度极差平均']=HumTem_data_normal[['温度1-平均变化', '温度2-平均变化', '温度3-平均变化', '温度4-平均变化', '温度5-平均变化']].mean(axis=1)
keep_cols=['ID_NUM', '日龄','外部-平均_mean','湿度内部平均_mean', 
           '鸡舍温度-最低_mean', '鸡舍温度-平均_mean', '鸡舍温度-最高_mean', '每日温差','探头温度标准差', 
          'Water','温度1-平均变化', '温度2-平均变化', '温度3-平均变化', '温度4-平均变化', '温度5-平均变化']
rename_col={
            '外部-平均_mean':'外部-平均',
            '湿度内部平均_mean':'湿度内部平均',
...
sliding_features_df.to_csv('./data/data_cleaned/wide_sliding_0602_2.csv', index=False,encoding='gbk')

wide_df=pd.read_csv('./data/data_cleaned/wide_sliding_0602_2.csv', encoding='gbk')
allinfo_dead=pd.read_csv('./data/data_cleaned/allinfo_dead0430.csv',encoding='gbk')
allinfo_dead['ID_NUM'] = allinfo_dead['ID_NUM'].apply(lambda x: 'G01' + x[3:] if isinstance(x, str) and x.startswith('G1A') else x)
allinfo_dead['ID_NUM'] = allinfo_dead['ID_NUM'].apply(lambda x: 'G04' + x[3:] if isinstance(x, str) and x.startswith('G1B') else x)
# 只有 G31_63,G28_22,匹不上，G27_24_H1，G27_25_H1为空
...
all_info_temdata2.to_csv('./data/data_cleaned/sliding_temdata0602_2.csv',index=False,encoding='gbk')
