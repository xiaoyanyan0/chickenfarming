...
def remove_columns_by_suffix(dataframe):
    # 创建空列表存储需要删除的列名
    columns_to_drop = []
    ...
    for col in dataframe.columns:
        ...
        try:
            # 提取最后两位字符
            last_two_chars = col[-2:]
            # 转换为整数
            number = int(last_two_chars)
            # 判断是否大于25
            if number > 25 or number < 0:
                columns_to_drop.append(col)
        except (ValueError, TypeError):
            # 如果无法转换为整数（如非数字字符），则跳过
            continue
    
    # 删除符合条件的列
    if columns_to_drop:
        dataframe = dataframe.drop(columns=columns_to_drop)
        print(f"已删除 {len(columns_to_drop)} 个字段: {columns_to_drop}")
    else:
        print("没有找到需要删除的字段")
    
    return dataframe
all_info_temdata2=remove_columns_by_suffix(all_info_temdata)
other_cols=['DOCdate', 'DOCAmount', 'HouseArea', 'Density', 'BirdsVariety', 'HESource', 'HEAge', 'Age',
            'Harveststatus', 'Houseid', 'HouseName', 'EstimatedSlaughterDate ', 'FarmSupervisor',
              'HouseAmount', 'ID_NUM', 'birds_placed', 'gender', 'house_area_m2', 'stocking_density', 
              'birds_hung', 'total_hung_weight_kg', 'avg_weight_kg', 'small_broilers_count', 
              'small_broilers_weight_kg', 'pp_dead_culled_count', 'dead_culled_weight_kg',
                'pp_rejects_count', 'pp_rejects_weight_kg', 'age_days', 'birds_caught_count', 
                'livability_pct', 'total_caught_weight_kg', 'yield_per_m2', 'final_avg_weight_kg', 
                'total_feed_kg', 'fcr', 'adjusted_fcr', 'eef', 'feed_cost', 'electricity_cost', 
                'gas_cost', 'depreciation_cost', 'chick_cost', 'total_cost', 'cost_per_kg', 'revenue', 
                'profit_per_house', 'feed_per_bird', 'electricity_per_bird', 'gas_per_bird', 'cost_per_bird',
                  'Mortality_rate']
...
high_missing=extract_high_missing_columns(all_info_temdata2, threshold=0.7)
all_info_temdata3=all_info_temdata2.drop(columns=high_missing.index,axis=1)


# 假设温度相关字段
temp_columns = [i for i in all_info_temdata3.columns if i not in other_cols]
len(temp_columns)
# 计算每行温度字段的缺失比例
all_info_temdata3['missing_ratio'] = all_info_temdata3[temp_columns].isnull().mean(axis=1)
all_info_temdata3['missing_ratio'].describe()
# 筛选缺失比例不超过50%的样本
valid_samples = all_info_temdata3[all_info_temdata3['missing_ratio'] <= 0.5].copy()
valid_samples['missing_ratio'].describe()
# 提取ID_NUM的前三位作为分组依据
valid_samples['group_key'] = valid_samples['ID_NUM'].str[:3]

# 按分组计算每个温度字段的平均值
group_means = valid_samples.groupby('group_key')[temp_columns].transform('mean')

# 用分组平均值填充缺失值
valid_samples[temp_columns] = valid_samples[temp_columns].fillna(group_means)

# 删除临时列
valid_samples = valid_samples.drop(columns=['missing_ratio', 'group_key'])


valid_samples.columns.to_list()
import toad
data_detect = toad.detector.detect(valid_samples)
data_detect['missing'].describe()
data_detect=data_detect.reset_index(drop=False)
data_detect.to_csv('./data/data_detected/valid_samples.csv',index=False,encoding='gbk')
# 使用示例
# cleaned_data = process_temperature_data(all_info_temdata)
valid_samples.columns.to_list()

date_columns = ['Harveststatus']
for col in date_columns:
    valid_samples[col] = pd.to_datetime(valid_samples[col])
    valid_samples[f'{col}_month'] = valid_samples[col].dt.month
    valid_samples[f'{col}_month']=valid_samples[f'{col}_month'].astype(str)


valid_samples=valid_samples.drop(date_columns,axis=1)
valid_samples.to_csv('./data/data_cleaned/valid_samples0519.csv', index=False, encoding='gbk')

winter_valid_samples=valid_samples[valid_samples['Harveststatus_month'].isin(['12', '1', '2'])]
