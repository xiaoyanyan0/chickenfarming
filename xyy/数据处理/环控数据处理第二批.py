        ...
        if re.match(r'^G(?:TF|\d{2})-\d{2}', farm_folder):
            ...
            for file in os.listdir(farm_path):
                if file.endswith('.xls') and 'H' in file:
                    ...
                    house_no = re.search(r'(H\d+)', file).group(1) if re.search(r'(H\d+)', file) else "Unknown"
                    ...
                    try:
                        ...
                        all_data.append(df[FINAL_COLUMNS])  # 只保留标准列
...
merge_df2.shape
merge_df2[merge_df2['id_no']=='G28-25']['house_no'].unique()



# root_folder='D:\\太阳谷\\chickenfarming\\data\\24.09-环控数据 2'
...
# merge_df3[merge_df3['id_no']=='G28-25']['house_no'].unique()
