...
all_info_temdata['Age_diff']=all_info_temdata.groupby('ID_NUM')['Age'].diff()

df2=all_info_temdata[(all_info_temdata['Age'] >= 10) & (all_info_temdata['Age'] <= 35) & (all_info_temdata['Age_diff']==1)]
...
cols = [i for i in df2.columns.to_list() 
            if ('water_per_diff_' in i) or ('water_per_shift_diff_' in i) ]
keep_col=cols+['Mortality_flg','Mortality_rate']
data=df2[df2['season']=='summer']
data.groupby('Mortality_flg')['Mortality_rate'].mean()
lgb_baseline, feature_imp, top_importantcol=lightgbm_modeling(data[keep_col], model_type='binary', random_state=42, quantile_threshold=0.8)
