...
all_info_temdata1=pd.read_csv('./data/data_cleaned/sliding_temdata0605.csv',encoding='gbk')
all_info_temdata2=pd.read_csv('./data/data_cleaned/sliding_temdata0602_2.csv',encoding='gbk')

cols=[i for i in all_info_temdata2.columns.to_list() if '平均变化_mean' in i]+['ID_NUM']
...
def process_seasonal_data(
    ...
    try:
        bin_table.to_csv(
            f"{output_path}{season}\\{season}_{target}_分箱.csv", 
            index=False, 
            encoding='gbk'
        )
    except IOError as e:
    ...
    try:
        ...
        fx.to_csv(
            f"{output_path}{season}\\{season}_{target}_分布.csv", 
            index=False, 
            encoding='gbk'
        )
...
def plot_all_variables(binned_file, stats_file, output_dir='.\\xyy\\模型分析\\',season='winter'):
    """绘制所有变量的图表"""
    import os
...
def plot_temp_eef(df, x_min=None, x_max=None, feature_name=""):
    ...
    return fig
def plot_all_variables_eef(binned_file, stats_file, output_dir='.\\xyy\\模型分析\\',season='winter'):
    """绘制所有变量的图表"""
    import os
    os.makedirs(output_dir, exist_ok=True)
...
importance_dict={
    "winter": {
        "MORTALITY_RATE": ["探头温度标准差_MEAN", "鸡舍温度-平均_MEAN", "温度4-平均_RANGE_MEAN"],
        "EEF": ["鸡舍温度-平均_MEAN", "湿度内部平均_MEAN", "探头温度标准差_MEAN"]
    },
    "autumn": {
        "MORTALITY_RATE": ["温度3-平均变化_MEAN", "探头温度标准差_MEAN", "温度2-平均_RANGE_MEAN"],
        "EEF": ["鸡舍温度-平均_MEAN", "湿度内部平均_MEAN", "外部-平均_MEAN"]
    },
    "spring": {
        "MORTALITY_RATE": ["湿度内部平均_MEAN", "外部-平均_MEAN", "温度2-平均变化_MEAN"],
        "EEF": ["鸡舍温度-平均_MEAN", "湿度内部平均_MEAN", "外部-平均_MEAN"]
    },
    "summer": {
        "MORTALITY_RATE": ["温度5-平均变化_MEAN", "湿度内部平均_MEAN", "鸡舍温度-平均_MEAN"],
        "EEF": ["温度3-平均_RANGE_MEAN", "鸡舍温度-平均_MEAN", "湿度内部平均_MEAN"]
    }
}
season='summer'
target_flg='Mortality_flg'
...
raw_data = pd.read_csv(f".\\xyy\\模型分析\\{season}\\{season}_{target}_分箱.csv", encoding='gbk')
...
optimized_data.to_csv(f".\\xyy\\模型分析\\{season}\\{season}_{target}_分箱_Optimized.csv", index=False, encoding='gbk')

if target=='MORTALITY_RATE':
    plot_all_variables(f".\\xyy\\模型分析\\{season}\\{season}_{target}_分箱_Optimized.csv",
                    f".\\xyy\\模型分析\\{season}\\{season}_{target}_分布.csv",
                    season=season)
elif target=='EEF':
    plot_all_variables_eef(f".\\xyy\\模型分析\\{season}\\{season}_{target}_分箱_Optimized.csv",
                    f".\\xyy\\模型分析\\{season}\\{season}_{target}_分布.csv",
                    season=season)
...
plot_all_variables(f".\\xyy\\模型分析\\{season}\\{season}_MORTALITY_RATE_分箱_Optimized1.csv",
                    f".\\xyy\\模型分析\\{season}\\{season}_MORTALITY_RATE_分布.csv",
                    season=season)
plot_all_variables_eef(f".\\xyy\\模型分析\\{season}\\{season}_EEF_分箱_Optimized1.csv",
                    f".\\xyy\\模型分析\\{season}\\{season}_EEF_分布.csv",
                    season=season)
...
df2[(df2['season']=='winter') & (df2['W3_21-23天_鸡舍温度-平均_MEAN']<24.3465271)]['MORTALITY_RATE'].mean()