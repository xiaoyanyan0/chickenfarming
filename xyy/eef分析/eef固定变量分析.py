...
quality_df=toad.quality(eef_df.drop(['ID_NUM','eef'],axis=1),target='eef_flg',iv_only=False)



from toad.plot import bin_plot
c=toad.transform.Combiner()
c.fit(eef_df.drop(columns=['ID_NUM','eef'],axis=1),y='eef_flg',min_samples=0.02)



transformed_df = c.transform(eef_df.drop(['ID_NUM'], axis=1), labels=True)
...
# 移除无关列（如id）
data = eef_df.drop(columns=['ID_NUM','eef'])
...
num_cols = ['DOCAmount', 'HouseArea', 'Density', 'Age', 'HouseAmount', ]
# 计算相关系数矩阵
