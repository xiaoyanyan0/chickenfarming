...
def get_dead_data(sheet, house_nos,DOCAmounts):
    ...
    for i, house in enumerate(house_nos):
        ...
        sub_data['DOCAmount'] = DOCAmounts[i]
    ...
    new_dead_data.columns = [
        ...
        'Fan_Num_More50','House_No','DOCAmount'
    ]
...
def read_transpose_excel(file_path):
    try:
        ...
        num=len(house_nos)+7-1
        amount_range = sheet2.range(f'C7:C{num}').value
        DOCAmounts = [no for no in amount_range if no]  
        #
        dead_data = get_dead_data(sheet, house_nos,DOCAmounts)
...
root_folders = [
                'D:\\太阳谷\\chickenfarming\\data\\catching date 24-03',
                # 'D:\\太阳谷\\chickenfarming\\data\\catching date 24-04',
                # 'D:\\太阳谷\\chickenfarming\\data\\catching date 24-05',
                # 'D:\\太阳谷\\chickenfarming\\data\\catching date 24-06',
                # 'D:\\太阳谷\\chickenfarming\\data\\catching date 24-07',
                # 'D:\\太阳谷\\chickenfarming\\data\\catching date 24-08'
                ]
...
# result_df_dead[result_df_dead['DOCAmount'].isna()]
# result_df=result_df[result_df['HouseNo']!='Total']
# result_df_dead[result_df_dead['ID_NUM']=='G30_64_H20']

result_df_dead.to_csv('./data/data_cleaned/daily_report_data0604.csv', index=False, encoding='gbk')