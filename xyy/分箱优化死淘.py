...
class AdvancedBinningOptimizer:
    ...
    def _find_merge_candidates(self, df):
        ...
        for i in range(1, len(df)):
            prev_eef = df.loc[current_group[-1], 'MORTALITY_RATE']
            curr_eef = df.loc[i, 'MORTALITY_RATE']
    ...
    def _merge_operation(self, df, merge_groups):
        ...
        for group in merge_groups:
            ...
            merged_record = {
                ...
                'MORTALITY_RATE': np.average(subset['MORTALITY_RATE'], weights=subset['Count']),
    ...
    def _generate_summary_row(self, df):
        ...
        total_iv = df['IV'].sum()
        avg_eef = np.average(df['MORTALITY_RATE'], weights=df['Count'])  # 按Count加权平均EEF
...
if __name__ == "__main__":
    raw_data = pd.read_csv(".\\xyy\\死淘分析\\output\\winter_饮水差值分箱.csv", encoding='gbk')
    
    optimizer = AdvancedBinningOptimizer(
        max_eef_diff=0.003,  # 死淘率差异阈值
        min_bin_size=0.05,
        min_iv_loss=0.1
    )
    
    optimized_data = optimizer.optimize(raw_data)
    optimized_data.to_csv(".\\xyy\\死淘分析\\output\\winter_饮水差值分箱_Optimized.csv", index=False, encoding='gbk')
...
class AdvancedBinningOptimizer:
    ...
    def _merge_operation(self, df, merge_groups):
        ...
        for group in merge_groups:
            ...
            merged_record = {
                ...
                'Count (%)': total_count / df['Count'].sum(),
                'JS': subset['JS'].sum() if 'JS' in subset.columns else 0,
                'Age_interval': subset['Age_interval'].iloc[0]  # 新增获取 Age_interval 值
            }
            merged_data.append(merged_record)
        return pd.DataFrame(merged_data)
    ...
    def optimize(self, raw_data):
        optimized_results = []
        age_intervals = raw_data['Age_interval'].unique()
        for age_interval in age_intervals:
            age_data = raw_data[raw_data['Age_interval'] == age_interval].copy()
            features = age_data['feature'].unique()
            for feat in features:
                feat_data = age_data[age_data['feature'] == feat].copy()
                cleaned_data = self._preprocess(feat_data)
                if len(cleaned_data) < 3:
                    optimized_results.append(cleaned_data)
                    continue
        ...
        # 合并所有特征数据并保留原始列序
        original_columns = raw_data.columns.tolist()
        if 'Age_interval' not in original_columns:
            original_columns.append('Age_interval')
        optimized_data = pd.concat(optimized_results, ignore_index=True)[original_columns]
        return optimized_data
    ...
    def _generate_summary_row(self, df):
        ...
        summary = pd.DataFrame({
            ...
            'Count (%)': [1.0],  # 汇总行占比为100%
            'JS': [df['JS'].sum()],  # 汇总JS值
            'Age_interval': df['Age_interval'].iloc[0]  # 新增获取 Age_interval 值
        })
        return summary

if __name__ == "__main__":
    raw_data = pd.read_csv(".\\xyy\\死淘分析\\output\\autumn_饮水差值分箱.csv", encoding='gbk')
    
    optimizer = AdvancedBinningOptimizer(
        max_eef_diff=0.00003,  # 死淘率差异阈值
        min_bin_size=0.05,
        min_iv_loss=0.1
    )
    
    optimized_data = optimizer.optimize(raw_data)
    optimized_data.to_csv(".\\xyy\\死淘分析\\output\\autumn_饮水差值分箱_Optimized.csv", index=False, encoding='gbk')
